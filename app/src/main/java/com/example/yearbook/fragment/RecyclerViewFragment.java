...
//        getActivity().findViewById(R.id.loadingPanel).setVisibility(GONE);
    ...
    private void initAdapter(final List<T> result) {
        try {
            adapter = adapterType.getDeclaredConstructor(List.class, ItemClickListener.class).newInstance(result, (ItemClickListener) (v, position) -> result.get(position).getOnClickListener(getContext()).onClick(v));
            initRecyclerView();
